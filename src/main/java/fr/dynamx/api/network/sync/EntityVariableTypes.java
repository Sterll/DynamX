...
// TODO port:1.20.1 - References to fr.dynamx.common.entities.* and fr.dynamx.common.network.sync.variables.*
// are forward declarations to types that will be ported in Phase 5 (network impls) / Phase 6 (entities).
// Forge's ByteBufUtils.{read,write}UTF8String replaced by inline length-prefixed UTF-8 helpers.
public enum EntityVariableTypes {
    ...
    PLAYER(Player.class, new EntityVariableSerializer<Player>() {
        @Override
        public void writeObject(ByteBuf buffer, Player object) {
            // TODO port:1.20.1 - Entity.getEntityId() -> Entity.getId() in Mojmap.
            buffer.writeInt(object == null ? -1 : object.getId());
        }

        @Override
        public Player readObject(ByteBuf buffer) {
            int id = buffer.readInt();
            if (id == -1)
                return null;
            // TODO port:1.20.1 - Minecraft.getMinecraft() -> Minecraft.getInstance(); world -> level.
    ...
    WHEELS_INFOS(CustomType.mapType(Byte.class, String.class), new EntityVariableSerializer<Map<Byte, String>>() {
        ...
        public Map<Byte, String> readObject(ByteBuf buf) {
            ...
            for (byte i = 0; i < size; i++) {
                currentValue.put(buf.readByte(), readUtf8(buf));
            }
            return currentValue;
        }
    }),
