    ...
    public void execute(IPhysicsWorld physicsWorld, PhysicsSpace dynamicsWorld, Set<PhysicsJoint> joints, HashSet<PhysicsEntity<?>> entities) {
        if (object != null) {
            switch (operation) {
                ...
                case ADD_OBJECT:
                    ...
                    if (physicsWorld.getWorld().isClientSide && object instanceof PhysicsRigidBody && ((PhysicsRigidBody) object).getUserObject() instanceof BulletShapeType && !((BulletShapeType<?>) ((PhysicsRigidBody) object).getUserObject()).getType().isTerrain()) {
                ...
                case ADD_ENTITY:
                    if (!entities.add((PhysicsEntity<?>) object)) {
                        DynamX.LOGGER.error("Entity " + object + " is already registered, please report this !");
                    }
                ...
                case REMOVE_ENTITY:
                    ...
                    entities.remove(et);
                    Runnable task = () -> {
                        List<PhysicsEntity> physicsEntities = et.level().getEntitiesOfClass(PhysicsEntity.class, et.getBoundingBox().inflate(10, 10, 10));
                    ...
                    DynamXMain.proxy.scheduleTask(et.level(), task);
                    break;
                case ADD_CONSTRAINT:
                    if (object != null && !joints.contains(object)) {
                        ...
                        DynamX.LOGGER.error("PhysicsJoint " + object + " is already registered, please report this !");
                    break;
        ...
        if (callback != null) {
            ...
            try {
                ...
                DynamX.LOGGER.error("Exception while executing callback of " + this + ". Callback: " + callback, e);
            }
        }
    }
