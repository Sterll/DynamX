...
public class PlayerPhysicsHandler {
    ...
    public PlayerPhysicsHandler(Player playerIn) {
        this.playerIn = playerIn;
        Quaternion localQuat = new Quaternion(0.0F, 1.0F, 0.0F, playerIn.getYRot());
        Transform localTransform = new Transform(new Vector3f((float) playerIn.getX(), (float) playerIn.getY() + 0.8f, (float) playerIn.getZ()), localQuat);
        ...
        switch (state) {
            ...
            case ACTIVATED:
                if (playerIn.isSpectator())
                    ...
                    position.set((float) playerIn.getX(), (float) playerIn.getY() + 0.8f, (float) playerIn.getZ());
                    if (Vector3f.isValidVector(position) && playerIn.fallDistance < 10) { //fixes a crash with elytra
                        bodyIn.setPhysicsLocation(position);
                        bodyIn.setPhysicsRotation(QuaternionPool.get().fromAngleNormalAxis((float) Math.toRadians(-playerIn.getYRot()), Vector3f.UNIT_Y));
    ...
    public void removeFromWorld(boolean delete, Level world) {
        removedCountdown = 30;
