...
public class VirtualTerrainFile {
    ...
    public void setChunk(VerticalChunkPos pos, List<ITerrainElement> elements) throws IOException {
        ...
        if (debug) {
            DynamX.LOGGER.info("[CHUNK DEBUG] Saving chunk " + pos + " with " + elements.size() + " elements in " + this);
        }
        lock(pos);
        ...
        for (ITerrainElement e : elements) {
            if (debug)
                DynamX.LOGGER.info("[CHUNK DEBUG] Saving element " + e);
    ...
    public List<ITerrainElement> loadChunk(VerticalChunkPos pos, ITerrainCache terrainCache) {
        boolean debug = DynamXConfig.enableDebugTerrainManager && DynamXConfig.chunkDebugPoses.contains(pos);
        if (debug) {
            DynamX.LOGGER.info("[CHUNK DEBUG] Loading chunk " + pos + " in " + this);
        }
        ...
        if (dt == null) {
            if (debug) {
                DynamX.LOGGER.error("[CHUNK DEBUG] Chunk not found in save file " + this);
            }
            unlock(pos);
            return null;
        }
        ...
        try {
            ...
            int size = in.readInt();
            if (debug)
                DynamX.LOGGER.info("[CHUNK DEBUG] Found " + size + " elements");
            for (int i = 0; i < size; i++) {
                ITerrainElement o;
                o = TerrainElementsFactory.getById(in.readByte());
                if (debug)
                    DynamX.LOGGER.info("[CHUNK DEBUG] Loading element " + o);
            ...
            DynamX.LOGGER.warn("Invalid terrain save version at " + pos + ", invalidating it... Error is " + e.getMessage());
            if (in != null) {
                try {
                    in.close();
            ...
            DynamX.LOGGER.error("Cannot load terrain save at " + pos + ", invalidating it...", e);
            if (in != null) {
                try {
                    in.close();
                } catch (IOException ignored) {
