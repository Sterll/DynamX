...
public class FileTerrainCache implements ITerrainCache {
    ...
    public FileTerrainCache(Level world) {
        // TODO port:1.20.1 - getSaveHandler().getWorldDirectory() removed; use ServerLevel.getServer().getWorldPath(LevelResource.ROOT) or similar
        Level serverWorld = world.isClientSide ? DynamXMain.proxy.getServerWorld() : world;
        assert serverWorld != null : "Terrain cannot be locally saved";
        File worldDir;
        if (serverWorld instanceof ServerLevel) {
            worldDir = ((ServerLevel) serverWorld).getServer().getWorldPath(net.minecraft.world.level.storage.LevelResource.ROOT).toFile();
        } else {
            worldDir = ServerLifecycleHooks.getCurrentServer().getWorldPath(net.minecraft.world.level.storage.LevelResource.ROOT).toFile();
        }
        assert worldDir != null : "World save dir not found";
        storageDir = new File(worldDir, "DnxChunks");
        storageDir.mkdirs();
    ...
    private void initV4(File slopes) {
        DynamX.LOGGER.debug("V4 terrain format detected !");
        try {
    ...
    private void initV3(File slopes) {
        DynamX.LOGGER.warn("V3 terrain format detected ! You should convert your slopes.");
    ...
    public void tick() {
        if (!dirtyChunks.isEmpty()) {
            ...
            // TODO port:1.20.1 - FMLCommonHandler removed; use ServerLifecycleHooks.getCurrentServer()
    ...
    public ChunkTerrain load(ChunkLoadingTicket ticket, Profiler profiler) {
        ...
        if (persistentElements != null && searchForDuplicatesAndRemove(pos, (List<ITerrainElement.IPersistentTerrainElement>) persistentElements)) {
            DynamX.LOGGER.info("Saving modified chunk " + ticket + " due to duplicated slopes");
    ...
    private boolean searchForDuplicatesAndRemove(VerticalChunkPos pos, List<ITerrainElement.IPersistentTerrainElement> elements) {
        ...
        if (!duplicates.isEmpty()) {
            DynamX.LOGGER.warn("Found " + duplicates.size() + " duplicates at " + pos + " ! Removing them...");
    ...
    protected void writeModifiedFiles() {
        DynamX.LOGGER.debug("Saving {} DynamX terrain files", terrainFileSaveQueue.size());
