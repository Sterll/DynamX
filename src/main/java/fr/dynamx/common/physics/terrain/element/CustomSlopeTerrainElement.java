...
public class CustomSlopeTerrainElement implements ITerrainElement.IPersistentTerrainElement {
    ...
    public void addDebugToWorld(Level mcWorld, Vector3f pos) {
        ...
        (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_SLOPE_BOXES : DynamXDebugOptions.SLOPE_BOXES).getDataIn().put(this.debugData.getUuid(), this.debugData);
    }

    @Override
    public void removeDebugFromWorld(Level mcWorld) {
        if (this.debugData != null)
            (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_SLOPE_BOXES : DynamXDebugOptions.SLOPE_BOXES).getDataIn().remove(this.debugData.getUuid());
    }
