...
public class CompoundBoxTerrainElement implements ITerrainElement {
    ...
    public void addDebugToWorld(Level mcWorld, Vector3f pos) {
        if (!debugData.isEmpty())
            (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_BLOCK_BOXES : DynamXDebugOptions.BLOCK_BOXES).getDataIn().putAll(debugData);
    }

    @Override
    public void removeDebugFromWorld(Level mcWorld) {
        for (Integer pos : debugData.keySet()) {
            (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_BLOCK_BOXES : DynamXDebugOptions.BLOCK_BOXES).getDataIn().remove(pos);
        }
    }
