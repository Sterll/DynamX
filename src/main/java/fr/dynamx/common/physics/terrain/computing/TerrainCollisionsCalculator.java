...
public class TerrainCollisionsCalculator {
    ...
    public static final ThreadLocal<Map<BlockState, IBlockCollisionBehavior>> behaviorLookup = ThreadLocal.withInitial(HashMap::new);
     ...
     * @see IBlockCollisionBehavior
     */
    public static List<ITerrainElement> computeCollisionFaces(VerticalChunkPos myPos, Level mcWorld, Profiler profiler, boolean debug) {
        ...
        //Root terrain
        TerrainBoxConstructor boxBuilder = new TerrainBoxConstructor(new AABB(x, y, z, x + 16, y + 16, z + 16), x, y, z, debug);
        //Compute all boxes
     ...
     */
    public static IBlockCollisionBehavior findBehavior(Map<BlockState, IBlockCollisionBehavior> lookup, BlockGetter world, BlockPos pos, BlockState forState) {
     ...
     */
    private static void canContinueAdding(Map<BlockState, IBlockCollisionBehavior> lookup, Level world, BlockPos pos, Direction.Axis axis, BlockState cur, BlockState onto, @Nullable BlockState lastStacked, TerrainCursor cursor, int ox, int oy, int oz, StackResult result) {
        if (printDebug)
     ...
     */
    private static boolean addRow(StackResult behavior, Map<BlockState, IBlockCollisionBehavior> lookup, Level world, TerrainBoxConstructor terrainBuilder, TerrainBoxBuilder currentBox, int x, int y, int z, TerrainCursor cursor, BlockPos.MutableBlockPos mutable, BlockState with) {
        BlockState cur = null, lastCur = null;
        int cz = currentBox.getZSize();
        for (int i = 0; i <= currentBox.getXSize(); i++) { //Along the x axis
            cur = world.getBlockState(mutable.set(x + i, y, z));
            canContinueAdding(lookup, world, mutable, Direction.Axis.Z, cur, with, lastCur, cursor, i, 0, cz + 1, behavior);
            if (printDebug)
        ...
        if (cur != null) {//If row can be added, add it !
            ...
            behavior.behavior.addBlockCollision(terrainBuilder, currentBox, cursor, world, mutable, cur, Direction.Axis.Z);
            if (currentBox.getZSize() == oldz) { //Avoid infinite loops...
                DynamX.LOGGER.error("Detected z stuck element at " + cursor + " " + mutable + " " + cur + " " + behavior + " " + currentBox + " " + terrainBuilder + ". Incrementing z size anyway...");
     ...
     * @return The behavior of this block only if this behavior is stackable in the given context, or none
     */
    private static boolean addPlane(StackResult behavior, Map<BlockState, IBlockCollisionBehavior> lookup, Level world, TerrainBoxConstructor terrainBuilder, TerrainBoxBuilder currentBox, int x, int y, int z, TerrainCursor cursor, BlockPos.MutableBlockPos mutable, BlockState with) {
        BlockState cur = null, lastCur = null;
        int cy = currentBox.getYSize();
        for (int i = 0; i <= currentBox.getXSize(); i++) { //Along the x axis
            for (int j = 0; j <= currentBox.getZSize(); j++) { //Along the z axis
                cur = world.getBlockState(mutable.set(x + i, y, z + j));
                canContinueAdding(lookup, world, mutable, Direction.Axis.Y, cur, with, lastCur, cursor, i, cy + 1, j, behavior);
                if (printDebug)
        ...
        if (cur != null) { //If plane can be added, add it !
            ...
            behavior.behavior.addBlockCollision(terrainBuilder, currentBox, cursor, world, mutable, cur, Direction.Axis.Y);
            if (currentBox.getYSize() == oldy) { //Avoid infinite loops...
                DynamX.LOGGER.error("Detected y stuck element at " + cursor + " " + mutable + " " + cur + " " + behavior + " " + currentBox + " " + terrainBuilder + ". Incrementing y size anyway...");
     ...
     */
    public static void loadBlockCollisions(Level world, TerrainBoxConstructor terrainBuilder) {
        AABB aabb = terrainBuilder.getSearchZone();
        ...
        // TODO port:1.20.1 - BlockPos.PooledMutableBlockPos removed; use plain MutableBlockPos
        BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
        //Local map to speed-up things
        Map<BlockState, IBlockCollisionBehavior> lookup = behaviorLookup.get();
        ...
        do {// tant qu'on n'est pas arrivé "en haut a droite" de notre box on continue : on
            ...
            BlockState boxStart = world.getBlockState(mutable.set(minX + cursor.dx, minY + cursor.dy, minZ + cursor.dz));

            //System.out.println("Begin at "+dx+" "+dy+" "+dz);
            stackable[0] = true;
            IBlockCollisionBehavior behavior = findBehavior(lookup, world, mutable, boxStart);
            // TODO port:1.20.1 - getMaterial().blocksMovement() replaced by blocksMotion()
            while (cursor.getHere() == 1 || !boxStart.blocksMotion() || !(stackable[0] = behavior.isStackableBlock(world, mutable, boxStart))) { // on ne repasse par sur des blocs ayant déjà
                ...
                boxStart = world.getBlockState(mutable.set(minX + cursor.dx, minY + cursor.dy, minZ + cursor.dz));
            ...
            if (cursor.dx + currentBox.getXSize() < lx) {
                ...
                cur = world.getBlockState(mutable.set(minX + cursor.dx + (currentBox.getXSize() + 1), minY + cursor.dy, minZ + cursor.dz));
                if (printDebug) {
                    System.out.println("==============");
                ...
                do {
                    canContinueAdding(lookup, world, mutable, Direction.Axis.X, cur, boxStart, null, cursor, currentBox.getXSize() + 1, 0, 0, stackResult);
                    if (printDebug)
                    ...
                    if (stackResult.behavior != null) {//If block can be added, add it !
                        if (stackResult.issue == StackIssue.STACKABLE) {
                            ...
                            behavior.addBlockCollision(terrainBuilder, currentBox, cursor, world, mutable, cur, Direction.Axis.X);
            ...
            if (cursor.dx == oldx && cursor.dy == oldy && cursor.dz == oldz) { //Avoid infinite loops...
                DynamX.LOGGER.error("Detected stuck element at " + cursor + " " + mutable + " " + boxStart + " " + behavior + " " + currentBox + " " + terrainBuilder + ". Incrementing cursor anyway...");
                cursor.incrXZY();
            }
        ...
        // TODO port:1.20.1 - PooledMutableBlockPos.release() removed; no-op
