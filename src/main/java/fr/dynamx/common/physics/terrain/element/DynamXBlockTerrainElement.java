...
public class DynamXBlockTerrainElement implements ITerrainElement {
    ...
    public DynamXBlockTerrainElement(int x, int y, int z, BlockPos pos) {
        ...
        this.y = y;
        this.z = z;
        // TODO port:1.20.1 - BlockPos copy constructor removed; use immutable() to copy
    ...
    @Override
    public PhysicsRigidBody build(Level world, Vector3f pos) {
        ...
        if (!(te instanceof TEDynamXBlock)) { //Not generated, should not happen because this should be removed from chunk
            DynamX.LOGGER.warn("[CHUNK DEBUG] Outdated DynamX block collisions found at: {}: TE not found. Maybe your packs have changed. The chunk will be reloaded", this.pos);
            return null;
        }
    ...
    @Override
    public void addDebugToWorld(Level mcWorld, Vector3f pos) {
        ...
        (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_BLOCK_BOXES : DynamXDebugOptions.BLOCK_BOXES).getDataIn().put(debugData.getUuid(), debugData);
    ...
    @Override
    public void removeDebugFromWorld(Level mcWorld) {
        if (debugData != null) {
            (mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_BLOCK_BOXES : DynamXDebugOptions.BLOCK_BOXES).getDataIn().remove(debugData.getUuid());
        }
    }
