...
public class TerrainFile extends VirtualTerrainFile {
    ...
    public TerrainFile(File container, boolean isSlopes) {
        if (DynamXConfig.enableDebugTerrainManager) {
            DynamX.LOGGER.info("Chunk debug pos are " + DynamXConfig.chunkDebugPoses + " // load " + container);
        }
        ...
        try {
            if (container.exists()) {
                ...
                if (isSlopes ? version == getVersion() : version >= 6) {
                    ...
                    in.close();
                    DynamX.LOGGER.warn("Outdated chunks collisions file : version " + version + ", everything will be erased !");
                    DynamX.LOGGER.info("Deleted with success : " + container.delete());
                } else {
                    in.close();
