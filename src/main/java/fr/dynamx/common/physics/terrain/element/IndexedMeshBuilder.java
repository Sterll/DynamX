...
public class IndexedMeshBuilder {
    ...
    public void addBoxes(List<MutableBoundingBox> boxes) {
        for (MutableBoundingBox boxAABB : boxes) {
            ...
            if (debugData != null) {
                ...
                {
                    //DynamX.LOGGER.warn("Found duplicated collision boxes ! Please report this to Aym' "+min+" "+max+" "+boxAABB+" "+boxes+" "+p);
    ...
    public void addStairBoxes(List<StairsBox> boxes) {
        for (StairsBox boxAABB : boxes) {
            Direction.Axis axis = boxAABB.getFacing().getAxis();
            min.set(x + (axis == Direction.Axis.Z ? boxAABB.getMin() : boxAABB.getMinOtherCoord()), y + boxAABB.getMinY(), z + (axis == Direction.Axis.X ? boxAABB.getMin() : boxAABB.getMinOtherCoord()));
            max.set(x + (axis == Direction.Axis.Z ? boxAABB.getMax() : boxAABB.getMinOtherCoord() + 1), y + boxAABB.getMinY() + 1, z + (axis == Direction.Axis.X ? boxAABB.getMax() : boxAABB.getMinOtherCoord() + 1));
