...
public class ChunkCollisions implements VerticalChunkPos.VerticalChunkPosContainer {
    ...
    public ChunkCollisions(Level mcWorld, VerticalChunkPos pos) {
        this.myPos = pos;
    ...
    public void reset() {
        ...
        if (state == EnumChunkCollisionsState.INVALID)
            DynamX.LOGGER.warn("[CHUNK DEBUG] Already re-setted chunk " + this);
        ...
        if (debug)
            DynamX.LOGGER.info("[CHUNK DEBUG] Resetting chunk " + getPos() + " with " + elements);
    ...
    public void addToBulletWorld(IPhysicsWorld physicsWorld, TerrainElementType terrainType, @Nullable Profiler profiler) {
        ...
        if (!elements.isEmpty()) //Body may be empty if the chunk is empty
        {
            ...
            //(mcWorld.isClientSide ? DynamXDebugOptions.CLIENT_CHUNK_BOXES : DynamXDebugOptions.CHUNK_BOXES).getDataIn().put(new BlockPos(myPos.x, myPos.y, myPos.z), new float[]{min.x, min.y, min.z, max.x, max.y, max.z, 0, 0, 1});
            updateNearEntities();
        }
        if (DynamXConfig.enableDebugTerrainManager)
    ...
    public void loadCollisionsSync(ITerrainManager manager, ITerrainCache cache, ChunkLoadingTicket ticket, Vector3f pos, Profiler profiler) {
        ...
        if (cached == null && cache.isRemoteCache()) {
            cache.asyncLoad(ticket, TerrainElementType.PERSISTENT_ELEMENTS).thenAccept(elements -> {
                ...
                DynamX.LOGGER.error("Failed to post-add persistent elements to chunk " + ticket, e);
                return null;
            });
        }
    }
    ...
    public CompletableFuture<Void> loadCollisionsAsync(ITerrainManager manager, ITerrainCache cache, ChunkLoadingTicket ticket, Vector3f pos) {
        ...
        return cache.asyncLoad(ticket, needType).thenAccept((terrainElements) -> { //Nb : the FileTerrainCache is not async, only the RemoteTerrainCache is
            ...
            if (!c.isValid()) {
                if (DynamXConfig.enableDebugTerrainManager) {
                    DynamX.LOGGER.warn("[COLL] Ignored async loaded chunk, new request sent {} {} Got {}", c.getTicket(), c.getSnapIndex(), terrainElements);
    ...
    private boolean localLoadCollisions(@Nullable ChunkTerrain cachedElements, ITerrainCache cache, TerrainElementType type, ChunkLoadingTicket ticket, Vector3f pos, Profiler profiler) {
        ...
        boolean debug = DynamXConfig.enableDebugTerrainManager && DynamXConfig.chunkDebugPoses.contains(getPos());
        if (debug) {
            DynamX.LOGGER.info("[CHUNK DEBUG] Computing collisions of chunk {} with {} before, and take from {} at {}", this, this.elements, ticket, System.currentTimeMillis());
        }
        if (DynamXConfig.enableDebugTerrainManager) {
        ...
        if (type != TerrainElementType.PERSISTENT_ELEMENTS) {
            if (cachedElements == null || cachedElements.getElements().isEmpty()) {
                ...
                shouldSave = true;
                if (debug) {
                    DynamX.LOGGER.info("[CHUNK DEBUG] Choice 1. Elements after : {} {} // Gave from param {}", this.elements.getElements(type), type, cachedElements != null ? cachedElements.getElements(TerrainElementType.ALL) : "null");
                }
            } else {
                this.elements.getElements().addAll(cachedElements.getElements());
                if (debug) {
                    DynamX.LOGGER.info("[CHUNK DEBUG] Choice 2. Elements after : {}", this.elements.getElements(type));
                }
            }
        }
        ...
        for (ITerrainElement iTerrainElement : this.elements.getElements()) {
            ...
            try {
                ...
                if (b == null) {
                    if (debug) {
                        DynamX.LOGGER.warn("[CHUNK DEBUG] Body of {} is null", iTerrainElement);
                    } else {
                        DynamX.LOGGER.debug("[CHUNK DEBUG] Body of {} is null", iTerrainElement);
                    }
                    shouldReload = true;
                    break;
                } else if (debug) {
                ...
                DynamX.LOGGER.error("[CHUNK DEBUG] Failed to add " + iTerrainElement + " in " + this, e);
                shouldReload = true;
                break;
            }
        }
        if (shouldReload) {
            ...
            DynamX.LOGGER.debug("[CHUNK DEBUG] Chunk {} has invalid data. Reloading it...", ticket);
            reset();
            setChunkState(EnumChunkCollisionsState.COMPUTING);
        ...
        this.elements.getPersistentElements().forEach(element -> {
            ...
            if (b == null) {
                DynamX.LOGGER.warn("[CHUNK DEBUG] Body of {} (persistent) is null", element);
            } else if (debug) {
    ...
    public void addPersistentElements(ITerrainManager manager, Collection<ITerrainElement.IPersistentTerrainElement> elementList) {
        boolean debug = DynamXConfig.enableDebugTerrainManager && DynamXConfig.chunkDebugPoses.contains(getPos());
        if (debug)
            DynamX.LOGGER.info("[CHUNK DEBUG] Adding elements to " + getPos() + " : " + elementList);
    ...
    public void removePersistentElements(ITerrainManager manager, Iterable<ITerrainElement.IPersistentTerrainElement> elements) {
        boolean debug = DynamXConfig.enableDebugTerrainManager && DynamXConfig.chunkDebugPoses.contains(getPos());
        if (debug)
            DynamX.LOGGER.info("[CHUNK DEBUG] Removing elements from " + getPos() + " : " + elements);
        ...
        DynamXMain.proxy.scheduleTask(mcWorld, () -> {
            try {
                ...
                if (mcWorld instanceof ServerLevel) {
                    int minX = (myPos.x - 1) * 16;
                    int maxX = (myPos.x + 2) * 16;
                    int minY = (myPos.y - 1) * 16;
                    int maxY = (myPos.y + 2) * 16;
                    int minZ = (myPos.z - 1) * 16;
                    int maxZ = (myPos.z + 2) * 16;
                    AABB aabb = new AABB(minX, minY, minZ, maxX, maxY, maxZ);
                    for (Entity e : mcWorld.getEntities(null, aabb)) {
                        if (e instanceof PhysicsEntity)
                            ((PhysicsEntity<?>) e).forcePhysicsActivation();
                    }
                }
            } catch (Exception e) {
                DynamX.LOGGER.error("Failed to update entities around " + getPos() + " after chunk change", e);
            }
        });
    }
