...
public class RemoteTerrainCache implements ITerrainCache {
    ...
    public RemoteTerrainCache(Level world) {
        this.world = world;
    }
    ...
    public void receiveChunkData(VerticalChunkPos pos, byte dataType, byte snapIdMod, @Nullable byte[] rawData) {
        if (!queries.containsKey(pos)) {
            ...
            DynamX.LOGGER.error("NO CORRESPONDING QUERY FOUND FOR " + pos + " " + dataType + " " + snapIdMod + " " + (rawData == null) + " at step: receiving data");
            return;
        }
        CompletableChunkLoading snap = queries.get(pos);
        if (snap.getSnapIdMod() != snapIdMod) {
            if (DynamXConfig.enableDebugTerrainManager)
                DynamX.LOGGER.error("PRE: Ignoring request answer " + snapIdMod + ". Now we want " + snap.getSnapIdMod() + " " + pos + ". Some data was " + (rawData == null));
            return;
        }
            ...
            try {
                ...
                for (int i = 0; i < size; i++) { //Read all received elements
                    ...
                    if (o.load(ITerrainElement.TerrainSaveType.NETWORK, in, pos)) {
                        if (o instanceof ITerrainElement.IPersistentTerrainElement) {
                            if (dataType == 0)
                                ...
                                DynamX.LOGGER.error("Persistent elements should be sent before normal elements. Data " + pos + " " + dataType + " " + o);
                        } else {
                            elements.add(o);
                        }
                    }
                    ...
                    if (start2 > DynamXConfig.networkChunkComputeWarnTime) {
                        DynamX.LOGGER.warn("Took " + start2 + " ms to load terrain from network at " + pos + " for element " + i + " of type " + o);
                    }
                }
                ...
                if (in != null) {
                    try {
                        ...
                        DynamX.LOGGER.error("I/O error closing data stream", e);
                    }
                }
                /*start = (System.currentTimeMillis() - start);
                if (start > DynamXConfig.networkChunkComputeWarnTime) {
                    DynamX.LOGGER.warn("Took " + start + " ms to load terrain from network at " + pos + " ! Loaded " + elements + " elements and " + persistents + " persistent elements");
                }*/
    ...
    private void receiveChunkData(VerticalChunkPos pos, byte dataType, byte snapIdMod, @Nullable byte[] rawData, @Nullable ChunkTerrain parsedData) {
        ...
        if (!queries.containsKey(pos)) {
            DynamX.LOGGER.error("NO CORRESPONDING QUERY FOUND FOR " + pos + " " + dataType + " " + (rawData == null) + " at step: handling data");
            return;
        }
        CompletableChunkLoading future = queries.get(pos);
        if (future.getSnapIdMod() != snapIdMod) { //Re-check validity
            if (DynamXConfig.enableDebugTerrainManager)
                DynamX.LOGGER.error("HD: Ignoring request answer " + snapIdMod + ". Now we want " + future.getSnapIdMod() + " " + pos + ". Some data was " + (rawData == null));
            return;
        }
        queries.remove(pos);
        if (dataType == 2) { //Persistent elements
            ...
            if (parsedData != null) { //Not empty : store received elements
                ...
                DynamX.LOGGER.error("Found an empty errored chunk at " + pos + " " + dataType + " " + (rawData == null));
    ...
    public ChunkTerrain load(ChunkLoadingTicket ticket, Profiler profiler) {
        if (dataCache.containsKey(ticket.getPos())) {
            ...
            DynamX.LOGGER.warn("[This is not an error] Using cached slopes at " + ticket);
