...
public abstract class BasePhysicsWorld implements IPhysicsWorld {
    ...
    public BasePhysicsWorld(Level world, boolean isRemoteWorld) {
        this.mcWorld = world;
    ...
    protected void flushOperations(Profiler profiler) {
        profiler.start(Profiler.Profiles.ADD_REMOVE_BODIES);
        {
            try {
                ...
                DynamX.LOGGER.error("[SCHED_ERR] There is " + scheduledTasks.size() + " real scheduled tasks remaining");
                DynamX.LOGGER.error("[SCHED_ERR] Error executing task !", e);
                try {
                    DynamX.LOGGER.error("[SCHED_ERR] Scheduled tasks : " + scheduledTasks);
                } catch (Exception exception) {
                    DynamX.LOGGER.error("[SCHED_ERR] Additional error while printing current tasks", e);
                }
                throw e;
            }
    ...
    protected void stepSimulationImpl(Profiler profiler, Semaphore syncThreadsLock) {
        ...
        } else if (mcWorld.getServer() != null && mcWorld.getServer().isDedicatedServer()) {
    ...
    public void schedule(Runnable r) {
        if (!scheduledTasksLock.get())
            ...
            DynamX.LOGGER.error("Adding physics tacks is locked due to a previous execution exception");
    }
    ...
    public void clearAll() {
        ...
        DynamXContext.getPhysicsWorldPerDimensionMap().remove(mcWorld.dimension());
        if(mcWorld.isClientSide) {
