...
/**
 * Manages vehicle doors as separate physics rigid bodies attached via 6-DoF joints.
 */
// TODO port:1.20.1 - PartDoor & sync variables/packets are forward references (Phase 4b/5). Side -> LogicalSide.
...
public class DoorsModule implements IPhysicsModule<AbstractEntityPhysicsHandler<?, ?>>, AttachModule.AttachToSelfModule,
    ...
    public Constraint createJoint(byte jointId) {
        ...
        PhysicsRigidBody doorBody = DynamXPhysicsHelper.fastCreateRigidBody(vehicleEntity, 40, doorShape, doorPos, vehicleEntity.getYRot());
        ...
        DynamXContext.getPhysicsWorld(vehicleEntity.level()).addCollisionObject(doorBody);
    ...
    public void spawnDoor(PartDoor door) {
        if (door.isEnabled()) {
            DynamXMain.proxy.scheduleTask(vehicleEntity.level(), () -> JointHandlerRegistry.createJointWithSelf(JOINT_NAME, vehicleEntity, door.getId()));
        }
    }
        ...
        if (simulatingPhysics) {
            for (byte doorID : attachedDoors.keySet()) {
                ...
                if (getCurrentState(doorID) == DoorState.OPENING && isDoorJointOpened(door, varContainer)) {
                    if (vehicleEntity.level().isClientSide && vehicleEntity.getSynchronizer().getSimulationHolder().isPhysicsAuthority(LogicalSide.CLIENT))
    ...
    /**
     * Legacy {@code onRemovedFromWorld} hook; 1.20.1 dispatches via Entity#onRemovedFromLevel / remove.
     */
    public void onRemovedFromWorld() {
        if (DynamXMain.proxy.shouldUseBulletSimulation(vehicleEntity.level())) {
            for (DoorPhysics body : attachedDoors.values())
                DynamXContext.getPhysicsWorld(vehicleEntity.level()).removeCollisionObject(body.doorBody);
            attachedDoors.clear();
        }
    }
