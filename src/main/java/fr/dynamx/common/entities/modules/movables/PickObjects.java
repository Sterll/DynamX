...
/**
 * Pick implementation: a Point2Point joint between the player's eye-ray and a hit body.
 */
// TODO port:1.20.1 - posX/posY/posZ -> getX/Y/Z; getEyeHeight() -> getEyeHeight() (still valid).
...
public class PickObjects extends MovableModule {
    ...
    private final EntityVariable<Player> mover = new EntityVariable<>((variable, value) -> {
    ...
    public void pickObject(Player playerPicking, PhysicsEntity<?> rayCastHitEntity, PhysicsRigidBody rayCastHitBody, Vector3f rayCastHitPos, float pickDistance) {
        if (mover.get() == null) {
            ...
            DynamXContext.getPlayerPickingObjects().put(playerPicking.getId(), rayCastHitEntity.getId());

            DynamXContext.getPhysicsWorld(playerPicking.level()).schedule(() -> JointHandlerRegistry.createJointWithSelf(JOINT_NAME, rayCastHitEntity, (byte) 0));
    ...
    public void unPickObject() {
        if (mover.get() != null && entity.getJointsHandler() != null) {
            DynamXContext.getPlayerPickingObjects().remove(mover.get().getId());
    ...
    public void preUpdatePhysics(boolean b) {
        ...
        Vector3f playerPosition = Vector3fPool.get(
                (float) mover.getX(),
                (float) mover.getY() + mover.getEyeHeight(),
                (float) mover.getZ());
