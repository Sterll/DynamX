...
/**
 * Boat-specific propeller + steering module.
 */
// TODO port:1.20.1 - BoatController/BoatPropellerInfo/BoatEngineInfo forward references (Phase 4b/7).
    ...
    public class BoatPropellerHandler implements IPackInfoReloadListener {
        ...
        public void onPackInfosReloaded() {
            if (hasEngine()) {
                ...
                gearBoxHandler = new AutomaticGearboxHandler.BoatGearBox((BoatPhysicsHandler<?>) entity.physicsHandler, this, gearBox);
            } else {
                engine = null;
                gearBox = null;
                gearBoxHandler = null;
        ...
        public void updateMovement() {
            ...
            if (isAccelerating()) {
                if (boatPhysicsHandler.getSpeedOnZAxisInBoatSpace() < -1f) {
                    brake(-1f);
                ...
                if (boatPhysicsHandler.getSpeedOnZAxisInBoatSpace() > 1) {
                    brake(1f);
                } else if (isEngineStarted()) {
                    accelerate(-1);
                } else {
