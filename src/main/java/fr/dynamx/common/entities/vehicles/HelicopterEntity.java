...
/**
 * Helicopter vehicle entity: seats (with FIXED-camera anonymous override), doors.
 */
// TODO port:1.20.1 - HelicopterPhysicsHandler/CameraMode/DynamXObjectLoaders forward (Phase 4b/7).
public class HelicopterEntity<T extends HelicopterPhysicsHandler<?>> extends BaseVehicleEntity<T> implements
    ...
    public HelicopterEntity(EntityType<? extends HelicopterEntity<?>> type, String name, Level world, Vector3f pos, float spawnRotationAngle, int metadata) {
        super(type, name, world, pos, spawnRotationAngle, metadata);
    }

    // TODO port:1.20.1 - Legacy (Level) constructor.
    public HelicopterEntity(Level level) {
        super(level);
    }
            ...
            seats = new SeatsModule(this, CameraMode.FIXED) {
                ...
                public void applyOrientationToEntity(Entity passenger) {
                    if (seats != null && seats.getControllingPassenger() == passenger && isMouseLocked()) {
                        passenger.setYRot(HelicopterEntity.this.getYRot());
                        passenger.yRotO = HelicopterEntity.this.yRotO;
                        passenger.setXRot(0);
                        passenger.xRotO = 0;
                        passenger.setYBodyRot(0);
                        passenger.setYHeadRot(passenger.getYRot() - HelicopterEntity.this.getYRot());
                    } else {
