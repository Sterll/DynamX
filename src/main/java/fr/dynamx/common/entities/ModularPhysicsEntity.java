...
public abstract class ModularPhysicsEntity<T extends AbstractEntityPhysicsHandler<?, ?>> extends PhysicsEntity<T> {
    ...
    public ModularPhysicsEntity(EntityType<? extends ModularPhysicsEntity<?>> type, Level level) {
        super(type, level);
    }

    public ModularPhysicsEntity(EntityType<? extends ModularPhysicsEntity<?>> type, Level level, Vector3f pos, float spawnRotationAngle) {
        super(type, level, pos, spawnRotationAngle);
    }

    // TODO port:1.20.1 - Legacy (Level) constructor. See PhysicsEntity for migration notice.
    public ModularPhysicsEntity(Level level) {
        super(level);
    }
    ...
    protected void getListenerModules() {
        ...
        moduleList.forEach(m -> {
            if (m instanceof IPhysicsModule.IEntityUpdateListener && ((IPhysicsModule.IEntityUpdateListener) m).listenEntityUpdates(getLogicalSide()))
                updateEntityListeners.add((IPhysicsModule.IEntityUpdateListener) m);
            if (m instanceof IPhysicsModule.IEntityPosUpdateListener && ((IPhysicsModule.IEntityPosUpdateListener) m).listenEntityPosUpdates(getLogicalSide()))
    ...
    public boolean initEntityProperties() {
        ...
        fireCreateModulesEvent(level().isClientSide ? Dist.CLIENT : Dist.DEDICATED_SERVER);
     ...
     */
    // TODO port:1.20.1 - Forge Side -> NeoForge Dist (logical side filtering by IGenericEvent is gone;
    // see PhysicsEntityEvent.CreateModules).
    protected abstract void fireCreateModulesEvent(Dist side);
    ...
    public void writeSpawnData(ByteBuf buffer) {
        ...
        // TODO port:1.20.1 - Forge IEntityAdditionalSpawnData removed; modules now expose their spawn
        // data via a duck-typed pair of writeSpawnData(ByteBuf)/readSpawnData(ByteBuf) methods (kept
        // for binary compatibility). Calls are routed via reflection until a clean Phase 5 hook lands.
        moduleList.forEach(m -> {
            try {
                m.getClass().getMethod("writeSpawnData", ByteBuf.class).invoke(m, buffer);
            } catch (NoSuchMethodException ignored) {
                // module has no spawn data
            } catch (Exception e) {
                throw new RuntimeException("Failed to write spawn data for module " + m, e);
            }
        });
    }
    ...
    public void readSpawnData(ByteBuf additionalData) {
        ...
        for (IPhysicsModule<?> m : moduleList) {
            ...
            try {
                m.getClass().getMethod("readSpawnData", ByteBuf.class).invoke(m, additionalData);
                i++;
            } catch (NoSuchMethodException ignored) {
                // module has no spawn data
            } catch (Exception e) {
                throw new RuntimeException("Failed to read spawn data for module " + m, e);
            }
        }
    }
    ...
    public void onPassengerTurned(Entity passenger) {
        // TODO port:1.20.1 - Was applyOrientationToEntity in 1.12. Renamed to onPassengerTurned in 1.20.
        if (this instanceof IModuleContainer.ISeatsContainer && ((IModuleContainer.ISeatsContainer) this).getSeats() != null) {
            // TODO port:1.20.1 - SeatsModule.applyOrientationToEntity is called by reflection (SeatsModule
            // is in this same Phase 6 batch; once compiled normally this can become a direct call).
            try {
                Object seats = ((IModuleContainer.ISeatsContainer) this).getSeats();
                seats.getClass().getMethod("applyOrientationToEntity", Entity.class).invoke(seats, passenger);
    ...
    public void positionRider(Entity passenger, Entity.MoveFunction callback) {
        // TODO port:1.20.1 - updatePassenger renamed to positionRider in 1.20.
        if (this instanceof IModuleContainer.ISeatsContainer && ((IModuleContainer.ISeatsContainer) this).getSeats() != null) {
            try {
                Object seats = ((IModuleContainer.ISeatsContainer) this).getSeats();
                seats.getClass().getMethod("updatePassenger", Entity.class).invoke(seats, passenger);
                return;
            } catch (Exception ignored) {
            }
        }
