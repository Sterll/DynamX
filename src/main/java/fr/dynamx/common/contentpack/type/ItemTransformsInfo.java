...
/**
 * Stores item transforms for different views ({@link net.minecraft.world.item.ItemDisplayContext}s)
 *
 * TODO port:1.20.1 - Original keyed the map by ItemCameraTransforms.TransformType (removed in 1.20.1).
 *   Now keyed by ItemDisplayContext.
 */
...
public class ItemTransformsInfo extends SubInfoType<AbstractItemObject<?, ?>> implements ISubInfoTypeOwner<ItemTransformsInfo> {
    private final Map<ItemDisplayContext, ViewTransformsInfo> viewTransforms = new HashMap<>();
    ...
    public ItemTransformsInfo(AbstractItemObject<?, ?> owner, float itemScale, Vector3f itemTranslate, Vector3f itemRotate) {
        this((ISubInfoTypeOwner<AbstractItemObject<?, ?>>) (ISubInfoTypeOwner<?>) owner);
    ...
    public void addViewTransforms(ItemDisplayContext viewType, ViewTransformsInfo viewTransformsInfo) {
        viewTransforms.put(viewType, viewTransformsInfo);
    }

    @Nullable
    public ViewTransformsInfo getViewTransforms(ItemDisplayContext viewType) {
