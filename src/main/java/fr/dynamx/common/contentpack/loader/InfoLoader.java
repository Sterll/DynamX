 ...
 *   - fr.dynamx.common.DynamXMain - replaced with DynamX.LOGGER.
 *   The reading/encoding/decoding plumbing is preserved verbatim. Note that
 *   ISubInfoTypeOwner.getInitiallyConfiguredProperties() was relaxed to List&lt;Object&gt; in
 *   Phase 3a, but the values stored are still PackFilePropertyData&lt;?&gt;; the unchecked cast in
 *   readInfo keeps the legacy behavior.
 */
public class InfoLoader<T extends ISubInfoTypeOwner<?>> extends InfoList<T> {
    ...
    protected void readInfo(SubInfoTypesRegistry<T> subInfoTypesRegistry, BufferedReader reader, INamedObject info) throws IOException {
        ...
        // TODO port:1.20.1 - getInitiallyConfiguredProperties() returns List<Object> after Phase 3a
        //   relaxation; values are PackFilePropertyData<?> so we copy them into the typed list.
        List<PackFilePropertyData<?>> foundProperties = new ArrayList<>();
        if (isSubInfoOwner) {
            for (Object o : obj.getInitiallyConfiguredProperties()) {
                if (o instanceof PackFilePropertyData)
                    foundProperties.add((PackFilePropertyData<?>) o);
            }
        }
        //Read the category
        String s;
    ...
    protected void encodeAObject(Map<String, byte[]> into, String objectKey, INamedObject object) {
        ...
        data.forEach((n, p) ->
        {
            try {
                ...
                if (e != null) {
                    sdata.append(((PackFilePropertyData) p).getType().toValue(e));
                } else {
                    sdata.append("null");
                }
                sdata.append("\n");
    ...
    public void receiveObjects(Map<String, byte[]> objects) {
        objects.forEach((o, d) -> {
            ...
            if (o.charAt(0) == '*' || o.charAt(0) == '-') {
                ...
                if (!infos.containsKey(of))
                    DynamX.LOGGER.warn("[PACK_SYNC] Cannot remove {} : not found x)", of);
                infos.remove(of);
            } else {
                throw new IllegalArgumentException("Wrong delta : " + o);
            }
        ...
        postLoad(true);
        DynamX.LOGGER.info("[PACK_SYNC] Synced {}", getPrefix());
    }
