 ...
 *
 * TODO port:1.20.1 - ContentPackDocGenerator / DocLocale (fr.dynamx.utils.doc) are not yet
 *   ported. The writeDocLine method now takes Object parameters and is a no-op stub until
 *   the doc package is ported.
 */
public class PackFilePropertyData<T> {
    ...
    /**
     * Writes a doc line for this property.
     *
     * TODO port:1.20.1 - DocLocale and ContentPackDocGenerator are not yet ported. Parameters
     *   are typed as Object until the doc package is ported. Method is a no-op stub.
     */
    public void writeDocLine(StringBuilder builder, Object locale, Object type) {
        // TODO port:1.20.1 - implement once fr.dynamx.utils.doc is ported.
        if (configFieldName == null || aliases == null) {
            return;
        }
        if (!configFieldName.equals(aliases[0])) {
            return;
        }
        // Stubbed body: keep aliases-aware formatting so the structure is observable.
        String sep = "|";
        builder.append(sep);
        builder.append(Arrays.stream(aliases).reduce((s, s2) -> s + ", " + s2).orElse("error"));
        builder.append(sep).append(this.type != null ? this.type.getTypeName() : "?").append(sep)
                .append(description == null ? "" : description).append(sep)
                .append(defaultValue == null || defaultValue.isEmpty() ? " - " : defaultValue).append(sep).append("\n");
    }
}
