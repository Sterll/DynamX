...
/**
 * Retrieves the public IP of the server (when behind a proxy), so DynamX UDP traffic can
 * be sent to the correct external address.
 */
public class ServerIPAdressRetriever {
    ...
    public void init() {
        if (DynamXConfig.usingProxy) {
            (new Thread(() -> ServerIPAdressRetriever.this.externalAddress = ServerIPAdressRetriever.this.retrieveExternalAddress(), "External Address Retriever Process")).start();
        }
    }
