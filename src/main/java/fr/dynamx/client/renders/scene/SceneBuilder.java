...
public class SceneBuilder<C extends IRenderContext, A extends IModelPackObject> {
     ...
     * If the parent node doesn't exist, the child node will be added to the root of the scene graph,
     * then the scene graph will be reordered when the parent node is added (ie you can add nodes in
     * any order). <br>
     * If the parent node is never added, an error will be reported and the node will be rendered
     * anyway.
     *
     * @param obj        The pack object that owns the scene graph, used for error reporting
     * @param part       The part that will be rendered at this node
    ...
    /**
     * Validates a node, and if it is invalid (no leaf is present), creates a fake leaf to avoid
     * crashes
     */
