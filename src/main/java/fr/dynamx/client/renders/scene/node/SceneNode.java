 ...
 * A scene graph is a tree of {@link SimpleNode} that can be rendered by a
 * {@link fr.dynamx.client.renders.RenderPhysicsEntity}. <br>
 * Each node match a {@link IDrawablePart} that will be rendered at the node position. <br>
 * Each node can have children nodes that will be rendered after the node itself, with the same
 * transformations.
 *
...
public interface SceneNode<C extends IRenderContext, A extends IModelPackObject> {
     ...
     * @param parentTransform The transformation matrix of the parent node, shouldn't be modified.
     *                        This <strong>isn't</strong> the transform of THIS node. <br> <br>
     ...
     * <strong>Note:</strong> unlike {@link SceneNode#render(IRenderContext, IModelPackObject, Matrix4f)},
     * the transformations of the parent nodes are not applied.
     *
     * @param context  The render context
     * @param packInfo The pack info of the entity (the owner of the scene graph)
     */
    ...
    /**
     * A node encapsulating another scene node, that can be used to listen and cancel the rendering
     * of the encapsulated scene node
     *
     * @param <C> The type of the render context
     * @param <A> The type of the pack info (the owner of the scene graph)
     */
