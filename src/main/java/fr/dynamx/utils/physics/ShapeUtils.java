    ...
    public static CompoundCollisionShape generateComplexModelCollisions(DxModelPath path, String objectName, Vector3f scale, Vector3f centerOfMass, float shapeYOffset) {
        ...
        if (dcInputStream != null) {
            ...
            try {
                ...
                DynamX.LOGGER.error("Cannot load .dc file of " + path + ". Re-creating it. Errored dc file was found in " + dcFilePackInfo, e);
        ...
        if (shapeGenerator == null) {
            ...
            long time = end - start;
            DynamX.LOGGER.info("Converted " + modelName + " model to shape and in " + time + " ms");
            ...
            if (!file.getPath().contains(".zip") && !file.getPath().contains(ContentPackLoader.PACK_FILE_EXTENSION)) { //not a zip pack
                ...
                DynamX.LOGGER.warn("Saving .dc file of " + modelPath + " of a zipped pack in " + file + ". Consider putting it in the zip file of the pack.");
                try {
                    ...
                    DynamX.LOGGER.info("Saved the shape " + file.getName());
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            ...
            time = end - start;
            DynamX.LOGGER.info("Generated " + file.getName() + " shape in " + time + " ms");
        }
        ...
        if (time > 10)
            DynamX.LOGGER.warn("Loaded " + dcFileLocation + " in " + time + " ms");
