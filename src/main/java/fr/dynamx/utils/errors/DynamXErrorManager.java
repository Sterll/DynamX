...
public class DynamXErrorManager {
    ...
    public static void printErrors(Dist side, ErrorLevel minLevel) {
        if(errorManager.getAllErrors().values().stream().anyMatch(e -> e.getHighestErrorLevel().ordinal() >= minLevel.ordinal())) {
            ...
            if(side.isClient())
                errorManager.printErrors(DynamX.LOGGER, Arrays.asList(INIT_ERRORS, PACKS_ERRORS, MODEL_ERRORS, ACsLib.getPlatform().getACsLibErrorCategory(), ACsGuiApi.getCssErrorType(), ModProtectionSystem.getMpsErrorCategory()), minLevel);
            else
                errorManager.printErrors(DynamX.LOGGER, Arrays.asList(INIT_ERRORS, PACKS_ERRORS, MODEL_ERRORS, ACsLib.getPlatform().getACsLibErrorCategory(), ModProtectionSystem.getMpsErrorCategory()), minLevel);
        }
    }
    ...
    private static String getErrorName(ErrorData errorData, boolean addStackTrace) {
       // TODO port:1.20.1 I18n.translateToLocal removed; using client-side I18n.get instead. Server-side calls will need a different approach.
       String translated = I18n.get("dynamx.error."+errorData.getGenericType());
